  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--muted)]">{tagline(doneTasks.length)}</p>
      </div>

      {doneTasks.length === 0 ? (
        <p className="text-sm text-[var(--faint)]">No tasks completed yet today.</p>
      ) : (
        <div className="space-y-2">
          {doneTasks.map((task) => (
            <div key={task.id} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--surface)] border border-[var(--border)]">
              <span className="text-[var(--green)]">✓</span>
              <span className="text-sm text-[var(--muted)] line-through">{task.content}</span>
            </div>
          ))}
        </div>
      )}
