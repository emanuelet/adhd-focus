...
describe("usePomodoro", () => {
	...
	it("accepts custom sprint length", () => {
		const { result } = renderHook(() => usePomodoro());
		act(() => {
			result.current.start("task-1", 25);
		});
	...
	it("accepts 90 minute sprint", () => {
		const { result } = renderHook(() => usePomodoro());
		act(() => {
			result.current.start("task-1", 90);
		});
	...
	it("accepts 120 minute sprint", () => {
		const { result } = renderHook(() => usePomodoro());
		act(() => {
			result.current.start("task-1", 120);
		});
	...
	it("break phase has correct duration for 52 min sprint", () => {
		const { result } = renderHook(() => usePomodoro());
		act(() => {
			result.current.start("task-1", 52);
		});
		act(() => {
	...
	it("calls onComplete handler when work phase ends", () => {
		...
		const handler = vi.fn();
		act(() => {
			result.current.setSprintCompleteHandler(handler);
		});
		act(() => {
			result.current.start("task-1", 25);
		});
