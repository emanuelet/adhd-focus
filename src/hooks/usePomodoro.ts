...
export function usePomodoro() {
	...
	const [sprintLength, setSprintLength] =
		useState<SprintLength>(DEFAULT_LENGTH);
	const ivl = useRef<ReturnType<typeof setInterval> | null>(null);
	const onCompleteRef = useRef<
		((taskId: string, length: SprintLength) => void) | null
	>(null);
	...
	const start = useCallback(
		(taskId: string, length: SprintLength = DEFAULT_LENGTH) => {
	...
	const totalSecs =
		phase === "work" ? workSecs : phase === "break" ? breakSecs : 0;
