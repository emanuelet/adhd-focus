...
function nullableString(value: unknown, name: string, maxLength = 500): string | null {
	if (value === null || value === undefined || value === "") return null;
	return stringValue(value, name, maxLength);
}

function stringArray(value: unknown, name: string): string[] {
	if (!Array.isArray(value) || value.length > 20) throw new Error(`Invalid organizer ${name}`);
	return value.map((entry) => stringValue(entry, name, 80));
}

export function validateCreateOrganizerItem(data: unknown): CreateOrganizerItemInput {
	if (!data || typeof data !== "object") throw new Error("Invalid organizer item");
...
export function validateOrganizerPayload(data: unknown): OrganizerProposalPayload {
	if (!data || typeof data !== "object") throw new Error("Invalid organizer proposal payload");
	const value = data as Record<string, unknown>;
	if (value.type === "todoist") {
		const action = value.action;
		if (action !== "create" && action !== "update") throw new Error("Invalid Todoist proposal action");
		const taskId = nullableString(value.taskId, "Todoist task id", 100);
		if (action === "update" && !taskId) throw new Error("Todoist updates require a task id");
		return {
			type: "todoist",
			action,
...
export function parseOrganizerResponse(response: string): { payload: OrganizerProposalPayload; rationale: string } {
	let value: unknown;
	try {
	...
	if (!value || typeof value !== "object") throw new Error("Organizer AI response was not an object");
...
export const createOrganizerProposal = createServerFn({ method: "POST" })
	.validator((data: unknown) => {
		if (!data || typeof data !== "object") throw new Error("Invalid organizer proposal input");
		return { itemId: stringValue((data as Record<string, unknown>).itemId, "item id", 100) };
	})
	.handler(async ({ data }) => {
		await requireAuth();
		const items = await db`SELECT id, text, url FROM organizer_items WHERE id = ${data.itemId}`;
		const item = items[0] as { id: string; text: string; url: string | null } | undefined;
		...
		const response = await openRouterClient.chat([
			...
			{ role: "user", content: JSON.stringify({ text: item.text, url: item.url }) },
		]);
    ...
    `;
		return { id, itemId: item.id, payload: parsed.payload, rationale: parsed.rationale, status: "pending" as const };
	});
...
export const applyOrganizerProposal = createServerFn({ method: "POST" })
	.validator((data: unknown) => {
		if (!data || typeof data !== "object") throw new Error("Invalid organizer apply input");
		const value = data as Record<string, unknown>;
		const decision = value.decision;
		if (decision !== "approve" && decision !== "reject") throw new Error("Invalid organizer decision");
		return { proposalId: stringValue(value.proposalId, "proposal id", 100), decision };
	})
	.handler(async ({ data }) => {
		await requireAuth();
		const rows = await db`SELECT * FROM organizer_proposals WHERE id = ${data.proposalId}`;
		const row = rows[0] as Record<string, unknown> | undefined;
		if (!row) throw new Error("Organizer proposal not found");
		if (row.status !== "pending") throw new Error("Organizer proposal is no longer pending");
